...
";
        ...
        public string Answer {
            get {
                return "There are {0} Lychrel numbers below ten thousand."
                    .FormatWith(this.Solve());
            }
        }

        public double Solve() {
            return 1.To(9999).Where(x => IsLychrel(x)).Count();
        }
        ...
        private bool IsLychrel(int n) {
            ...
            for (int i = 0; i < 50; i++) {
                num += BigInteger.Parse(new String(num.ToString().Reverse().ToArray()));
                if (num.IsPalindrome()) {
                    return false;
                }
            }
